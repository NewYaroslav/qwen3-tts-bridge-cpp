...
FakeTensor/ProxyTensor/runtime-assert `sympy` helper branches, plus
`librosa`/SciPy/joblib, out of the narrow Qwen Nuitka graph unless a profile
explicitly needs those paths. The profile nofollow rules intentionally target
only known eager-unused symbolic helper imports, not the whole `torch.fx`
